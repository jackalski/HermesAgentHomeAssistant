...
> Original commits: <https://github.com/techartdev/OpenClawHomeAssistant/commits/main>
...
> **Note**: Since Hermes Agent v2026.2.22+ `hermes-agent config get` redacts sensitive values (returns `hermes_redacted`). Read the token directly from the config file with `jq` as shown above.
...
> **Important (v2026.2.21+):** Hermes Agent now requires a **secure context** (HTTPS or localhost) for the Control UI. Plain HTTP over LAN is no longer accepted. The add-on's `access_mode` option makes this easy — see below.
>
...
> **Note**: Since Hermes Agent v2026.2.22+ `hermes-agent config get` redacts sensitive values — use `jq` to read directly from the config file.
...
| `http_proxy` | string | _(empty)_ | Optional outbound proxy URL for HTTP/HTTPS requests from Hermes Agent and Node tools. Example: `http://192.168.2.1:3128` |
...
> **Since Hermes Agent v2026.2.21**, the Control UI requires a secure context (HTTPS or localhost). Use the `access_mode` option for easy setup.
...
Or via terminal:
```sh
hermes-agent config set gateway.http.endpoints.chatCompletions.enabled true
```
...
| Symptom | Fix |
|---|---|
| `mcporter: command not found` | Run `hermes-agent onboard` first, then restart the add-on |
...
**Checks**:
1. Is the gateway running? In the terminal: `hermes-agent gateway status`
2. Is the bind mode correct? `hermes-agent config get gateway.bind` — must be `lan` for direct LAN access, or `loopback` if using `lan_https` mode
3. Is the port correct? `hermes-agent config get gateway.port`
...
**Symptom**: Logs repeat `Invalid config at /config/.hermes/hermes.json` and `tools.web.search.provider: web_search provider is not available: brave`.

**Cause**: The persisted Hermes Agent config selects the Brave web search provider, but that provider plugin is not currently installed or enabled in the add-on runtime.
...
```sh
jq 'del(.tools.web.search.provider)' /config/.hermes/hermes.json > /tmp/hermes.json && mv /tmp/hermes.json /config/.hermes/hermes.json
```
...
**Cause**: Hermes Agent v2026.2.21+ requires HTTPS or localhost. Plain HTTP over LAN is blocked. (v2026.2.22 further hardens this by defaulting remote onboarding to `wss://` and rejecting insecure non-loopback targets.)
...
**Cause**: Hermes Agent v2026.2.21+ checks the browser's `Origin` header against an allow-list. When using the built-in HTTPS proxy (`lan_https`), the origin (`https://<ip>:<port>`) must be registered in `gateway.controlUi.allowedOrigins`.
...
4. **Manual override** (advanced, from the add-on terminal):
   ```sh
   hermes-agent config set gateway.controlUi.allowedOrigins '["https://192.168.1.10:18789"]'
   ```
...
**Cause**: Hermes Agent v2026.2.21+ requires new devices to complete a pairing handshake before the Control UI WebSocket is accepted. Loopback connections are auto-approved (v2026.2.22 further improves this with loopback scope-upgrade auto-approval), but LAN connections (including those through the HTTPS proxy) require explicit approval.
...
> **v2026.2.22 note:** The gateway now logs a security warning on startup when this flag is active. The warning is expected and harmless — run `hermes-agent security audit` for details.
...
2. If the error persists, set it manually:
   ...
   ```
   Then restart the gateway: `hermes-agent gateway restart`
3. Alternatively, approve devices individually without disabling auth:
   ```sh
   hermes-agent devices list       # show pending pairing requests
   hermes-agent devices approve <requestId>
   ```
...
> **Note**: Since Hermes Agent v2026.2.22+ `hermes-agent config get` redacts sensitive values (returns `hermes_redacted`). Use `jq` to read the token directly from the config file.
...
**Symptom**: In add-on terminal, commands that open direct gateway WebSocket (for example some `hermes-agent status`/gateway probes) fail with unauthorized and logs mention `trusted_proxy_user_missing`.
...
**Symptom**: Hermes Agent's `web_fetch` tool (or any outbound HTTP call from a skill) fails with `fetch failed`.
...
**Fix**: Ensure `force_ipv4_dns` is **true** (default since v0.5.51). If you upgraded from an older version, the option may still be set to `false` — change it to `true` in **Settings → Add-ons → Hermes Agent → Configuration** and restart.
...
Restart the add-on — it will generate a fresh config. You'll need to run `hermes-agent onboard` again.
...
Hermes Agent supports multiple agent profiles. Configure them via `hermes-agent configure` or by editing `/config/.hermes/hermes.json`. The gateway serves all configured agents.
...
Run `hermes-agent configure` in the terminal to reconfigure your AI providers, or edit `/config/.hermes/hermes.json` directly. You can use OpenAI, Google (Gemini), Anthropic (Claude), local models, and more.
